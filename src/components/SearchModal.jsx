...
const SearchModal = ({ title, toggleHandler, close }) => {
  ...
  */
  ...
  return (
    <div>
      {toggleHandler() && (
        <div id="modal-bg" class='h-full w-full bg-slate-700/60 items-center flex justify-center absolute z-20 top-0 left-0'>
          <div id="inside-box" class='h-[45rem] w-[70rem] rounded-[12px] bg-slate-500 text-white p-6 flex flex-col gap-y-3'>
            <div id="top" class='flex gap-x-1'>
              <button class='bg-transparent hover:bg-slate-600/60 py-1 px-2 rounded-md transition' onclick={() => close()}>
